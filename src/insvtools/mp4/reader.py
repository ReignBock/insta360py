...
class Track:
    ...
    def sample_times(self) -> list[float]:
        ...
        for sample in self.samples:
            times.append(current / self.timescale)
...
def _entries(payload: bytes, offset: int, count: int, fmt: struct.Struct) -> list[tuple]:
    return [fmt.unpack_from(payload, offset + i * fmt.size) for i in range(count)]
...
def _read_stsz(payload: bytes) -> tuple[int, list[int]]:
    ...
    return 0, [_U32.unpack_from(payload, 12 + 4 * i)[0] for i in range(count)]
...
def _read_stz2(payload: bytes) -> tuple[int, list[int]]:
    ...
    if field_size == 4:
        ...
        sizes = [struct.unpack_from(">H", data, 2 * i)[0] for i in range(count)]
    else:
...
def _read_chunk_offsets(box: Box, payload: bytes) -> list[int]:
    (count,) = _U32.unpack_from(payload, 4)
    if box.type == b"co64":
        return [_U64.unpack_from(payload, 8 + 8 * i)[0] for i in range(count)]
    return [_U32.unpack_from(payload, 8 + 4 * i)[0] for i in range(count)]
...
def _read_track(f: BinaryIO, trak: Box) -> Track:
    ...
    # Expand sample-to-chunk into per-chunk sample counts.
    samples: list[Sample] = []
    chunk_sample_description: list[int] = []
    sample_index = 0
            ...
            for _ in range(samples_per_chunk):
                ...
                samples.append(
                    Sample(
                        ...
                        if sample_index < len(durations)
                        else 0,
                        cts_offset=cts_offsets[sample_index] if cts_offsets else 0,
                        # stss is 1-based; no stss means every sample is a sync sample.
                        sync=(sample_index + 1) in sync_samples
