...
recording, and leaves out folders with no footage. The list appears as soon as the folders are searched, and each recording shows Reading until its markers are found. It skips hidden folders,
...
```bash
uv sync --extra dev
uv run pytest --cov                  # 435 tests, 100% coverage (enforced)
