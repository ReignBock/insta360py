...
def workdir(sample_insv: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Run each test in its own directory, beside a fresh sample."""
...
def test_dump_meta_matches_java(workdir: Path, golden: Path) -> None:
    """dump-meta JSON must match the reference byte for byte."""
...
def test_extract_meta_matches_java(workdir: Path, golden: Path) -> None:
    """The extracted trailer must match the reference exactly."""
...
def test_decompose_meta_matches_java(workdir: Path, golden: Path) -> None:
    """Every per-frame file must match the reference exactly."""
    assert main(["decompose-meta", "sample.insv"]) == 0
...
def test_decompose_meta_single_frame_type(workdir: Path, golden: Path) -> None:
    """--frame-type writes just that frame."""
    assert main(["decompose-meta", "--frame-type=10", "sample.insv"]) == 0
...
def test_decompose_then_compose_restores_the_file(workdir: Path) -> None:
    """Splitting a trailer into frames and rebuilding it is lossless."""
    original = (workdir / "sample.insv").read_bytes()

    assert main(["decompose-meta", "sample.insv"]) == 0
    assert main(["remove-meta", "sample.insv"]) == 0
    assert (workdir / "sample.insv").stat().st_size < len(original)

    assert main(["compose-meta", "sample.insv"]) == 0
    assert (workdir / "sample.insv").read_bytes() == original


def test_extract_then_replace_restores_the_file(workdir: Path) -> None:
    """Extracting a trailer and putting it back is lossless."""
    original = (workdir / "sample.insv").read_bytes()

    assert main(["extract-meta", "sample.insv"]) == 0
    assert main(["remove-meta", "sample.insv"]) == 0
    assert main(["replace-meta", "sample.insv"]) == 0

    assert (workdir / "sample.insv").read_bytes() == original


def test_remove_meta_leaves_a_plain_mp4(workdir: Path) -> None:
    """Stripping the trailer leaves a file with no signature."""
    assert main(["remove-meta", "sample.insv"]) == 0
